...
class NmapDocker(NmapDev):
    ...
    def __init__(self,d_cmds,pass_lst,ip_key_dic):
        ...
        # self.docker_cmd_list = ["docker ps|awk '/->22/ {print $(NF-1)}'|awk -F '->' '{print $1}'|awk -F ':' '{print $NF}'"]
        self.docker_cmd_list = ["docker ps |awk -F '->' '/->22/ {print $1}'|grep -v 'CONTAINER'|awk 'BEGIN{FS~/s+/;}{print $NF\" \"$1\" \"$2;}'|sed s/0.0.0.0://"]
    ...
    def do_nmap(self,host_list):
        ...
        for ip_item in ip_items:
            ...
            port_list = res["docker ps |awk -F '->' '/->22/ {print $1}'|grep -v 'CONTAINER'|awk 'BEGIN{FS~/s+/;}{print $NF\" \"$1\" \"$2;}'|sed s/0.0.0.0://"].decode().split("\n")
            # port_list = res["docker ps|awk '/->22/ {print $(NF-1)}'|awk -F '->' '{print $1}'|awk -F ':' '{print $NF}'"].decode().split("\n")
