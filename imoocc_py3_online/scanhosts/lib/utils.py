...
def mac_trans(mac):
    ...
    if mac:
        mac_lst = mac.split("\n".encode())
        mac_res = [item.replace(b":",b'').replace(b"000000000000",b'').replace(b"00000000",b'') for item in mac_lst]
...
class prpcrypt():
    ...
    def decrypt(self,text):
        ...
        print('eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee',text)
        text='6284acc9fb69ca5ea53d2431ca73b2ab'
        plain_text  = cryptor.decrypt(a2b_hex(text))
        # plain_text  = cryptor.decrypt(text.strip())
        return plain_text
